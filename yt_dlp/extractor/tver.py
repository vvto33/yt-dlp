...
class TVerIE(InfoExtractor):
    ...
    def _real_initialize(self):
        session_info = self._download_json(
            'https://platform-api.tver.jp/v2/api/platform_users/browser/create', None, 'Creating session',
            data=b'device_type=pc', headers={'Content-Type': 'application/x-www-form-urlencoded'})
        self._PLATFORM_QUERY = traverse_obj(session_info, ('result', {
            'platform_uid': 'platform_uid',
            'platform_token': 'platform_token',
        }))

    def _call_platform_api(self, path, video_id, note=None, **kwargs):
        return self._download_json(
            f'https://platform-api.tver.jp/service/api/{path}', video_id, note, **kwargs)

    def _series(self, series_id):
        series_info = self._call_platform_api(
            f'v2/callSeries/{series_id}', series_id, 'Downloading season info',
            headers=self._HEADERS, query=self._PLATFORM_QUERY)
        series_title = traverse_obj(series_info, ('result', 'content', 'content', 'title'))

        episode_urls = []

        season_info = self._download_json(
            f'https://service-api.tver.jp/api/v1/callSeriesSeasons/{series_id}', series_id,
            'Downloading season info', headers=self._HEADERS)
        for season_id in traverse_obj(
                season_info, ('result', 'contents', lambda _, s: s['type'] == 'season', 'content', 'id')):
            episode_info = self._call_platform_api(
                f'v1/callSeasonEpisodes/{season_id}', season_id, 'Downloading episode info',
                headers=self._HEADERS, query=self._PLATFORM_QUERY)
            episode_urls.extend(traverse_obj(episode_info, (
                'result', 'contents', lambda _, e: e['type'] == 'episode',
                'content', 'id', {lambda e: f'https://tver.jp/episodes/{e}'})))

        return self.playlist_from_matches(episode_urls, series_id, series_title, ie=TVerIE)
    ...
    def _real_extract(self, url):
        ...
        episode_info = self._call_platform_api(
            f'v1/callEpisode/{video_id}', video_id, 'Downloading episode info',
            fatal=False, headers=self._HEADERS, query={
